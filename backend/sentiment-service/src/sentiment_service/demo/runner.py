...
class Runner:
    ...
    def run_ticker_level(self, hourly: HourlyLevelScore):
        ...
        if self.ticker.get(ticker, None) is None:
            ...
            logger.info("Ticker Level Score for %s has been created", ticker)
...
def read_hourly_history(path: Path) -> List[Dict[str, Any]]:
    ...
    if not isinstance(data, list):
        raise ValueError(
            "Expected ticker_sentiment_hourly.json to be a JSON array of hourly rows."
        )
    return data
...
def run(
    ...
    *,
    ...
    from sentiment_service.Ticker_level_score import (
        HourlyRow,
        compute_signal_for_ticker,
        normalize_hourly_row,
    )
    ...
    for rec in kept:
        text = rec["text_normalized"].strip() or (
            rec["title"] + "\n\n" + rec["text"]
        ).strip()
    ...
    created_times = [
        e["created_at_utc"] for e in kept if isinstance(e.get("created_at_utc"), int)
    ]
    ...
    for cur in hour_rows:
        hist_rows = [
            normalize_hourly_row(h) for h in history if h.get("ticker") == cur["ticker"]
        ]
